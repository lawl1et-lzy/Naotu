    ...
    <el-dialog title="权限" :visible.sync="dialogFormVisible">
      <el-form :model="form" ref="form" :rules="rules">
        <el-form-item label="权限名称" :label-width="formLabelWidth" prop="name">
          <el-input v-model="form.name" autocomplete="off" placeholder="请输入权限名称"></el-input>
        </el-form-item>
  ...
  methods: {
    ...
    async fetchFormData(id) {
      try {
        ...
        const doc = await Api.identityFind(rp)
    ...
    async fetchTableData() {
      try {
       ...
       if(doc) {
         this.handleIdentityFindData(doc)
       }
      } catch (error) {
    ...
    async fetchSave() {
      try {
        const doc = await Api.identityCreate(this.form)
        const { response } = doc
        if(!response.error_code) {
          this.message('保存成功')
    ...
    async fetchUpdate() {
      try {
        const doc = await Api.identityUpdate(this.form)
        const { response } = doc
        if(!response.error_code) {
          this.message('更新成功')
          this.dialogFormVisible = false
